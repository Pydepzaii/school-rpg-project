        ...
        else
        {
            ...
            { // Đồng bộ tốc độ 0.04 giây/chữ
                ...
                if (letterCount < (int)strlen(fullText))
                {
                    ...
                    // --- [MỚI] BỘ ĐẾM NHỊP ÂM THANH INTRO ---
                    static int introSoundTick = 0;
                    introSoundTick++;

                    // Cứ 3 ký tự (0.12s) phát tiếng 1 lần
                    if (introSoundTick % 3 == 0)
                    {
                        Audio_PlaySoundEffect(SFX_TALK); // Dùng SFX_TALK thay vì tiếng tạch tạch cũ
                    }
                }
                else
                {
